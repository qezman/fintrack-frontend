...
export const RegisterForm = () => {
  ...
  const validateField = (field: keyof FieldErrors, value: string) => {
    ...
    if (field === "email") {
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value))
        updated.email = "Invalid email address";
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        {apiError && (
          <div className="bg-(--expense-dim) border border-[rgba(248,81,73,0.3)] rounded-lg px-3.5 py-2.5 text-(--expense) text-sm leading-relaxed">
            {apiError}
          </div>
        )}
        ...
        <p className="text-center text-sm text-[var(--text-secondary)]">
          ...
          <Link
            to="/login"
            className="text-[var(--neutral)] hover:underline font-medium"
          >
            Sign in →
          </Link>
        </p>
